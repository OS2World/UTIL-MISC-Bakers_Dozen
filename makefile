...
# /* $subtitle:  'Elipse Ltd.  [jms]         Revised: 2005 Jul 05' */
#
...
!IFDEF Debug
CompOpt =   /Tm+i+x+ /O- /C /Fl$(TMP)icc.lst /L+j+s+p58i- /G4m+e+f+d-h+ /Q /Wall+gen-ppt-uni-trd-ext-ppc- /W3 /Re /V"NotOptim" /DDEBUG=1
LinkOpt =   /NoFreeFormat /Base:0x10000 /ExePack /NoE /Align:4 /PM:vio /Debug /Map
ExtraObj =  \IbmCpp\lib\cppopa3.obj ### \IbmCpp\lib\SetArgV.Obj \DevTools\PMPrintf\printf.obj
AllLib =    so32dll tcp32dll Elipse GpfP2CIS ErrorMgr
!ELSE
...
ExtraObj =  empty.obj ### \IbmCpp\lib\SetArgV.Obj
AllLib =    so32dll tcp32dll Elipse GpfP2CIS
!ENDIF
...
##############################################################################
#                                                                            #
#       Change the following lines as needed (also the 'Debug' define)       #
#                                                                            #
##############################################################################
...
showtime.exe:   runfix.obj

showtime.obj:   runfix.c
